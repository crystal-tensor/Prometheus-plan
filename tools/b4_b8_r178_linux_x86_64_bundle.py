#!/usr/bin/env python3
"""Create a hash manifest for the downloadable R178 evidence bundle."""
...
BUNDLE_PATH = "results/B4_B8_R178_linux_x86_64_bundle_manifest_v0.json"
...
def payload_hash(payload: dict[str, Any]) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError("R178 bundle source payload mismatch")
    return str(observed)
...
def main() -> int:
    ...
    if (root / BUNDLE_PATH).exists():
        raise ValueError("R178 bundle manifest already exists")
    ...
    required = [
        ...
        Path(RESULT_PATH),
        Path(ORACLE_PATH),
        Path("research/B4_B8_R178_linux_x86_64_replay.md"),
        Path("research/B4_B8_R178_independent_linux_x86_64_oracle.md"),
    ]
    required.extend(
    ...
    for relative in required:
        path = root / relative
        if not path.is_file():
            raise ValueError(f"R178 bundle artifact missing: {relative}")
        artifacts.append(
            {
                "path": str(relative),
    ...
    manifest = {
        "title": "B4/B8/B10 R178 Linux x86-64 downloadable evidence bundle",
        "version": 0,
        "method": "b4_b8_r178_linux_x86_64_bundle_v0",
    ...
    if (
        ...
        raise ValueError("R178 bundle worker or GitHub-run boundary mismatch")
