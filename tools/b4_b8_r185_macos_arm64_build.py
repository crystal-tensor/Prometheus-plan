#!/usr/bin/env python3
"""Build the preregistered R185 Qiskit accelerator on macOS arm64."""
...
METHOD = "b4_b8_r185_macos_arm64_replication_build_v0"
...
MANIFEST_PATH = "research/source_lineage/Qiskit_2_4_1_R185_window_exact_macos_arm64_build_manifest.json"
...
EXPECTED_SOURCE_HASHES = {
    ...
    "crates/transpiler/src/passes/vf2/vf2_layout.rs": (
        "372c8084c29d31946f83138e68861de437d74d19fb992d23fa74213ea044096b"
    ),
}
...
def validate_preregistration(
    ...
    if int(time.time()) <= created:
        raise ValueError("R185 build must start after public preregistration")
    public = contract["public_preregistration"]
    if args.preregistration_discussion != public["discussion"]:
        raise ValueError("R185 preregistration discussion boundary mismatch")
    if args.preregistration_created_at != public["created_at"]:
        raise ValueError("R185 preregistration creation time mismatch")
    if contract.get("execution_started") is not False:
        raise ValueError("R185 contract must remain an unopened record")
    current_commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=root, text=True
    ).strip()
    if args.preregistration_commit != current_commit:
        raise ValueError("R185 runner commit does not match the preregistration commit")
    if subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=root, text=True
    ).strip():
        raise ValueError("R185 build requires a clean public-main worktree")
    remote_main = subprocess.check_output(
        ["git", "ls-remote", "origin", "refs/heads/main"], cwd=root, text=True
    ).split()[0]
    if remote_main != current_commit:
        raise ValueError("R185 runner commit is not the current public main")
    ...
    if ancestor.returncode != 0:
        raise ValueError("R185 build commit predates the public design commit")
...
def run_step(
    ...
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R185 build step failed: {step_id}\n{completed.stdout}\n{completed.stderr}"
        )
    return record
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("R185 contract identity mismatch")
    ...
    for relative in contract.get("result_paths_must_be_absent", []):
        if (root / relative).exists():
            raise ValueError(
                f"R185 evidence existed before macOS execution: {relative}"
            )
    for relative in contract.get("build_output_paths_created_before_replay", []):
        if (root / relative).exists():
            raise ValueError(f"R185 build output existed before build: {relative}")
    if platform.system() != "Darwin" or platform.machine() != "arm64":
        raise ValueError(
            f"R185 requires macOS arm64, observed {platform.system()} {platform.machine()}"
        )
    ...
    build_started_at = datetime.now().astimezone().isoformat()

    source = Path("/tmp/prometheus-r185-window-exact-qiskit-source")
    if source.exists():
    ...
    if observed_source_hashes != EXPECTED_SOURCE_HASHES:
        raise ValueError("R185 patched source hashes do not match the frozen patch")
    ...
    if library_name != "qiskit_pyext" or "cdylib" not in crate_types:
        raise ValueError(
            "R185 Qiskit pyext metadata does not declare qiskit_pyext cdylib"
        )
    ...
    candidate = source / "target/release" / f"lib{library_name}.dylib"
    if not candidate.is_file():
        raise ValueError(
            f"R185 source-declared release artifact is missing: {candidate}"
        )
    ...
    if not all(
        ...
        for marker in ("Mach-O", "arm64", "dynamically linked shared library")
    ):
        raise ValueError(f"R185 built unexpected binary: {file_description}")

    qiskit_spec = importlib.util.find_spec("qiskit")
    if qiskit_spec is None or not qiskit_spec.submodule_search_locations:
        raise ValueError("R185 cannot locate the pinned installed Qiskit package")
    installed_qiskit = Path(
        next(iter(qiskit_spec.submodule_search_locations))
    ).resolve()
    smoke_overlay = Path("/tmp/prometheus-r185-window-exact-import-smoke")
    ...
    for installed_accelerator in smoke_package.glob("_accelerate*"):
        if not installed_accelerator.is_file():
            continue
    ...
    if (
        ...
        or not all(import_smoke.get("r184_entry_points", {}).values())
    ):
        raise ValueError("R185 Python import smoke did not bind the built accelerator")

    manifest = {
        "title": "Qiskit 2.4.1 R185 windowed-exact-score macOS arm64 build manifest",
        "version": 0,
        "method": METHOD,
        "status": "macos_arm64_pyext_built_and_imported_after_preregistration",
        "preregistration": {
        ...
        "public_runner_attestation": {
            "mode": "clean_public_main_local_runner",
            "runner_commit": runner_commit,
            "remote_main_at_build_start": remote_main,
            "clean_worktree_before_build": True,
            "platform_contract_passed": True,
            "build_started_at": build_started_at,
            "build_completed_at": datetime.now().astimezone().isoformat(),
        },
        "build_steps": steps,
