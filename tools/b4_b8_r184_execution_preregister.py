#!/usr/bin/env python3
"""Bind R184 execution tooling after the public design Discussion exists."""
...
OUTPUT_PATH = "benchmarks/B4_B8_R184_window_exact_score_execution_contract_v0.json"
REPORT_PATH = "research/B4_B8_R184_window_exact_score_execution_contract.md"
TOOL_PATHS = [
...
def binding(root: Path, relative: str) -> dict[str, Any]:
    path = root / relative
    if not path.is_file():
        raise ValueError(f"R184 execution binding is missing: {relative}")
...
def main() -> int:
    ...
    for relative in (OUTPUT_PATH, REPORT_PATH):
        if (root / relative).exists():
            raise ValueError(f"R184 execution output already exists: {relative}")
    if not args.discussion.startswith(
        "https://github.com/crystal-tensor/Prometheus-plan/discussions/"
    ):
        raise ValueError("R184 public Discussion URL is outside the project repository")
    created = datetime.fromisoformat(args.created_at.replace("Z", "+00:00"))
    if created.timestamp() >= time.time():
        raise ValueError("R184 public Discussion timestamp is not in the past")
    ...
    if design.get("protocol_payload_hash") != protocol_hash:
        raise ValueError("R184 design protocol binding mismatch")
    if set(design["required_before_execution"]) != set(TOOL_PATHS):
        raise ValueError("R184 design and execution tool lists differ")
    ...
    if subprocess.check_output(
        ...
        raise ValueError(
            "R184 execution contract must be generated from the clean public design commit"
        )
    ...
    contract: dict[str, Any] = {
        "title": "B4/B8/B10 R184 windowed-exact-score execution contract",
        "version": 0,
        "contract_id": "B4-B8-R184-windowed-exact-score-execution-contract-v0",
        "status": "execution_tooling_bound_unopened",
        ...
        "build_output_paths_created_before_replay": [
            ...
            "research/source_lineage/R184_window_exact_linux_x86_64_build_logs",
        ],
