...
Current state: **stage 1z evidence hardening; B1/B7 has now closed the simple same-envelope `cone_01` phase-removal route with 0 exact-pass windows, B7 still lacks a certified occurrence-removing rewrite, B2 remains a hardware-like leakage negative boundary, and B10 remains a denominator-boundary track**.
...
|---|---|---|---|---|
| B1 | Hardware-aware quantum circuit compression | Virtual-SWAP plus U3 phase-factored T-resource diagnostics; `gcm_h6` selector counts 270 arbitrary decimal rotations; cone-feasibility isolates `cone_01` with 35 pair-local single-arbitrary windows; restricted phase-removal tests on all 35 windows give 0 exact passes for remove-only, fixed Z-phase, and continuous RZ replacement, best residual 0.36435162331705345. | Replace the failed simple phase-absorption route with a broader two-qubit semantic rewrite, KAK/Clifford scaffold, or alternate certificate-bearing transformation for at least 30 `cone_01` windows, then re-run B7 only after certificates exist. | Demote if reductions disappear on broader benchmarks or certificate generation cannot scale beyond local rewrite evidence. |
...
| B7 | Fault-tolerance co-design | B1/B2 bridge plus FT synthesis ledger; `w8_21` claim-boundary closure with 43480 optimizer runs, 0 exact replacements, and 0 ledger removal; template-priority gate over 12 retained templates; B1-side `cone_01` phase-removal gate shows the simple same-envelope route has 0 exact-pass windows, so no B7 saving is countable yet. | Improve the B7 minimum row through a broader certified `cone_01` occurrence-removing rewrite for `gcm_h6`, or produce a scoped symbolic obstruction/proof; do not count the phase-removal gate as a resource delta. | Demote if B1/B2-linked reductions collapse once physical layout, factory throughput, feed-forward, and occurrence-level synthesis certificates are explicit. |
