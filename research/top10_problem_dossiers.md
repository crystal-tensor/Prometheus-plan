...
B9 update: T-B9-004m repairs the Lean source interface, builds the pinned Lean 4.12.0/Mathlib environment, and records a source-backed zero-returncode transcript for Lean, Lake, and the B9 module. The checked-run acquisition gate passes 7/7 and the priority transcript packet passes 9/9. This supports a checked indexed interface only; the all-n open-boundary Hamiltonian construction, unconditional spectral lemmas, and any Quantum PCP/NLTS/global no-go result remain open.
